            ...
            for(int l = 0; l <= targetLen; l++) {
                for(int s = 0; s <= targetSum; s++) {
                    ...
                    for(int f = 0; f <= freq[k]; f++) {
                        ...
                        if(newlen > targetLen || newsum > targetSum) continue;
                        int tmp = (1LL * curr[l][s]) % mod;
                            tmp = (1LL * tmp * inv[f]) % mod;
                            tmp = (1LL * tmp * inv[freq[k] - f]) % mod;

                        next[newlen][newsum] = (next[newlen][newsum] + tmp) % mod;
                    }
                }
            }
            
            std::swap(curr, next);
            std::fill(&next[0][0], &next[0][0] + sizeof(next) / sizeof(next[0][0]), 0);
        }

        ways = (1LL * ways * curr[targetLen][targetSum]) % mod;
        return ways;
    }
};
